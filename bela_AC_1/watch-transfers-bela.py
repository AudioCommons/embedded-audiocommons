#-----------------------------------------------------------------------
# This python script watches for new files in the folder "sound_files".
# When there is a new file, the python script "move-transferred-sound.py" is launched.
# Code inspired from:
...
def ticktack():
    print "tick tack"
    subprocess.Popen(["python", "move-transferred-sound.py"])

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s - %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S')
    path = sys.argv[1] if len(sys.argv) > 1 else 'sound_files'
